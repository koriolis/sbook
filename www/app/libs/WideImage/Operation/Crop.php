 ...
 * @copyright 2007-2010
  ...
  **/
	...
	class WideImage_Operation_Crop
	{
		...
		{
			$width = WideImage_Coordinate::fix($img->getWidth(), $width, false);
			$height = WideImage_Coordinate::fix($img->getHeight(), $height, false);
			$left = WideImage_Coordinate::fix($img->getWidth(), $left, false, $width);
			$top = WideImage_Coordinate::fix($img->getHeight(), $top, false, $height);
			if ($left < 0)
			{
				$width = $left + $width;
				$left = 0;
